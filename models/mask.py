...
class ConvAttention(nn.Module):
    def __init__(self, dim, num_heads):
        ...
        # CvT convolution projection to generate QKV
        self.qkv = nn.Conv2d(
            dim, dim * 3,
    ...
    def forward(self, x):
        ...
        # Generate QKV: [N, C, H, W] -> [N, 3C, H, W]
        qkv = self.qkv(x)
        # Split into Q, K, V: -> [N, C, H, W], [N, C, H, W], [N, C, H, W]
        q, k, v = qkv.chunk(3, dim=1)

        # Expand to multi-head: [N, C, H, W] -> [N, num_heads, C_head, HW]
